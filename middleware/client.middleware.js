...
const { editFields,  createCustomError, createResponse, emailInvoice } = require('../utils/app.utils')
...
const confirmClientInvoice = asyncHelper(async (req, _, next) => {
  ...
  if(!client) return next(createCustomError(404, 'Client does not exist'))
...
const populateInvoice = singArgAsync( async( id, _ ) => {
